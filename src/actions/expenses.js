...
export const startAddExpense = (expenseData = {}) => {  // default to {} if expenseData not received
    ...
    return (dispatch, getState) => {  // this function only works because we setup the middleware using redux in configureStore.js
        ...
        return database.ref(`users/${uid}/expenses`).push(expense).then((ref) => {      // .then gets called with reference that we can access via ref
...
export const startRemoveExpense = ( {id} = {} ) => {  // no arg needed as we'll be fetching expenses;  
    ...
    return (dispatch, getState) => {
        ...
        return database.ref(`users/${uid}/expenses/${id}`)   // gets the specific expense from firestore; 
...
export const startEditExpense = ( id, updates ) => {    // updates is an object 
    ...
    return (dispatch, getState) => {
        const uid = getState().auth.uid;  
        return database.ref(`users/${uid}/expenses/${id}`)   // gets the specific expense from firestore; return needed so we can add .then() later
...
export const startSetExpenses = () => {  // no arg needed as we'll be fetching expenses;  
    ...
    return (dispatch, getState) => {  // this function only works because we setup the middleware using redux in configureStore.js
        const uid = getState().auth.uid;   // getState is 2nd var in redux async function in line above;  
        return database.ref(`users/${uid}/expenses`)   // create an array from database info using .once() & childSnapshot; 
