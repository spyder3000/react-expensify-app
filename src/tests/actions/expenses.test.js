...
beforeEach((done) => {
    ...
    database.ref('expenses').set(expensesData).then(() => done()); // ensures that .forEach doesn't allow test case to run until Firebase updates
})
...
test('Should Remove Expense from firebase', (done) => {
    ...
    store.dispatch(startRemoveExpense({ id })).then(() => {
        ...
        return database.ref(`expenses/${id}`).once('value');  // return from this should be null 
    }).then((snapshot) => {
